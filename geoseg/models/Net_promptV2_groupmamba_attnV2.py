...
class PromptGenBlock(nn.Module):
    def __init__(self,prompt_dim=128,prompt_len=5,prompt_size = 96,lin_dim = 192):
        ...
        self.spatial_attn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(lin_dim//4, 1, kernel_size=1, bias= False),
            nn.Sigmoid()
        )
        self.prompt_param = nn.Parameter(torch.rand(1,prompt_len,prompt_dim,prompt_size,prompt_size))
        # 2、改进prompt的参数初始化方式
        # self.prompt_param = nn.Parameter(torch.empty(1, prompt_len, prompt_dim, prompt_size, prompt_size))
        # nn.init.xavier_uniform_(self.prompt_param)   #  使用Xavier初始化
    ...
    def forward(self,x):
        ...
        x_attended = x * attn_map

        emb = x_attended.mean(dim=(-2,-1))
        # 生成 Prompt 权重
