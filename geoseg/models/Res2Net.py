...
class Bottle2neck(nn.Module):
    ...
    def forward(self, x):
        ...
        spx = torch.split(out, self.width, 1)       # 按通道拆分为scale个分支（每个分支width通道）
...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth=26, scale=4, num_classes=2):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):    # 处理卷积层
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            elif isinstance(m, nn.BatchNorm2d):   # 处理批量归一化层
                nn.init.constant_(m.weight, 1)
                nn.init.constant_(m.bias, 0)

    # 用于构建 “层组”（Layer Group）的核心函数
