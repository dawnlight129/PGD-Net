...
def overlock_xt(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_url = 'https://github.com/LMMMEng/OverLoCK/releases/download/v1/overlock_xt_in1k_224.pth'
        logging.info(f"Downloading pretrained weights from {pretrained_url}")
        torch.hub.download_url_to_file(pretrained_url, 'overlock_xt.pth')
        checkpoint = torch.load('overlock_xt.pth', map_location='cpu')
...
def overlock_t(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_url = '/root/EB-TDFNet/pre_trained_weights/overlock_t_in1k_224.pth'
        if os.path.exists(pretrained_url):
            checkpoint = torch.load(pretrained_url, map_location='cpu')
            unexpected_keys = [k for k in checkpoint.keys() if 'aux_head' in k or 'head' in k]
            ...
            pretrained_url = 'https://github.com/LMMMEng/OverLoCK/releases/download/v1/overlock_t_in1k_224.pth'
            # logging.info(f"Downloading pretrained weights from {pretrained_url}")
            torch.hub.download_url_to_file(pretrained_url, 'overlock_t_in1k_224.pth')
            checkpoint = torch.load('overlock_t_in1k_224.pth', map_location='cpu')
            unexpected_keys = [k for k in checkpoint.keys() if 'aux_head' in k or 'head' in k]
            for k in unexpected_keys:
                checkpoint.pop(k)
...
def overlock_s(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_url = 'https://github.com/LMMMEng/OverLoCK/releases/download/v1/overlock_s_in1k_224.pth'
        logging.info(f"Downloading pretrained weights from {pretrained_url}")
        torch.hub.download_url_to_file(pretrained_url, 'overlock_s.pth')
        checkpoint = torch.load('overlock_s.pth', map_location='cpu')
        model.load_state_dict(checkpoint['state_dict'])
    return model
...
def overlock_b(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_url = '/root/EB-TDFNet/pre_trained_weights/overlock_t_in1k_224.pth'
        logging.info(f"Downloading pretrained weights from {pretrained_url}")
        torch.hub.download_url_to_file(pretrained_url, 'overlock_b.pth')
        checkpoint = torch.load('overlock_b.pth', map_location='cpu')
        model.load_state_dict(checkpoint['state_dict'])
    return model

if __name__ == "__main__":
    model = overlock_t(pretrained=True)
