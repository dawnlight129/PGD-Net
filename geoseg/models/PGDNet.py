...
class PromptGenBlock(nn.Module):
    def __init__(self,prompt_dim=128,prompt_len=5,prompt_size = 96,lin_dim = 192):
        ...
        # self.conv1 = nn.Conv2d(lin_dim, lin_dim//2,kernel_size=1,stride=1,padding=0,bias=False)
        self.local_weight = nn.Sequential(
            nn.Conv2d(lin_dim, lin_dim, kernel_size=1,stride=1,padding=0,bias=False),
            nn.GELU(),
            nn.AdaptiveAvgPool2d(1)
        )
    def forward(self,x):
        B,C,H,W = x.shape
        print(f"输入特征x的尺寸: B={B}, C={C}, H={H}, W={W}")  # 看输入特征尺寸
        emb = x.mean(dim=(-2,-1))
        print(f"GAP输出emb尺寸: {emb.shape}")  # GAP输出尺寸

        # 生成 Prompt 权重
        prompt_weights1 = F.softmax(self.linear_layer(emb),dim=1)
        print(f"prompt_weights1尺寸: {prompt_weights1.shape}")  # 看全局权重尺寸

        local_weights = self.local_weight(x)
        print(f"CAP模块（local_weight）自适应池化输出尺寸: {local_weights.shape}")  # 重点：自适应池化输出尺寸
        local_weights = rearrange(local_weights, 'b d 1 1 -> b d')

        prompt_weights2 = F.softmax(self.linear_layer(local_weights),dim=1)
        print(f"prompt_weights2尺寸: {prompt_weights2.shape}")

        # 打印prompt_param的尺寸
        print(f"self.prompt_param尺寸: {self.prompt_param.shape}")  # 看prompt_param的初始化尺寸
        ...
        print(f"prompt1尺寸: {prompt1.shape}, prompt2尺寸: {prompt2.shape}")
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        weight = self.sigmoid(self.conv(torch.cat([avg_out, max_out], dim=1)))     # 拼接两个特征
        x_conv = self.conv_skip(x)
        out = x_conv + x * weight
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.reduce_level3 = nn.Conv2d(int(decode_channels * 2 ** 3), int(decode_channels * 2 ** 2), kernel_size=1,bias=bias)
        self.reduce_level2 = nn.Conv2d(int(decode_channels * 2 ** 2), int(decode_channels * 2 ** 1), kernel_size=1,bias=bias)
        self.reduce_level1 = nn.Conv2d(int(decode_channels * 2 ** 1), int(decode_channels * 2 ** 0), kernel_size=1,bias=bias)
        self.Block1 = nn.Sequential(BasicConv2d(decode_channels * 2 ** 2, decode_channels * 2 ** 2, 1),DWConv(decode_channels * 2 ** 2))
        self.Block2 = nn.Sequential(BasicConv2d(decode_channels * 2 ** 1, decode_channels * 2 ** 1, 1),DWConv(decode_channels * 2 ** 1))
        self.Block3 = nn.Sequential(BasicConv2d(decode_channels * 1, decode_channels * 1, 1),DWConv(decode_channels * 1))
    ...
    def forward(self, x, res1, res2, res3, res4, h, w):
        ...
        inp_dec_level2 = self.Block2(inp_dec_level2)
        ...
        inp_dec_level1 = self.Block3(inp_dec_level1)
...
if __name__ == '__main__':
    ...
    model = PGDNet(decode_channels=64, num_classes=2, pretrained=True).cuda()
    output = model(input)
