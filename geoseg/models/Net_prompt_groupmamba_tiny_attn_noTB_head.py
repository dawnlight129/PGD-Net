...
class Attention_n(nn.Module):
    def __init__(self, dim, num_heads, bias):
        super(Attention_n, self).__init__()
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        # print(f"打印x.shape: {x.shape}")
        B, C, H, W = x.shape               # [1, 256, 64]   dim=64, num_heads=8, sr_ratio=2
        N = H * W
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, 
        ...
        self.attn_n = Attention_n(dim, num_heads, bias)
...
class FinalPatchExpand_X4(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0,3,1,2).contiguous()     # [1, 128, 64, 64]
        x = self.out(x)
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.seg_outs = nn.Conv2d(decode_channels, decode_channels, 1, 1) 
        self.final_patch_expand = FinalPatchExpand_X4(dim=decode_channels,num_classes=num_classes)
    ...
    def forward(self, x, res1, res2, res3, res4, h, w):
        ...
        # print(f"打印inp_dec_level3{inp_dec_level3.shape}")
        ...
        ###   
        inp_dec_level1 = self.spatial_attn(inp_dec_level1)  # 对融合后的特征应用空间注意力模块 , [2, 64, 128, 128]
        # print(inp_dec_level1.shape)
        out = self.seg_outs(inp_dec_level1)
        out = self.final_patch_expand(out)
        # out = self.segmentation_head(inp_dec_level1)
        # out = F.interpolate(out, size=(h, w), mode='bilinear', align_corners=False)
        # print('输出解码shape',out.shape)  # torch.Size([2, 2, 512, 512])
        return out
