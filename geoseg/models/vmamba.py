...
class PatchMerging2D(nn.Module):
    ...
    def forward(self, x):
        ...
        if (W % 2 != 0) or (H % 2 != 0):
            ...
            SHAPE_FIX[1] = W // 2
        # 四叉树分块（非重叠下采样）
        x0 = x[:, 0::2, 0::2, :]  # 左上：偶行偶列
        x1 = x[:, 1::2, 0::2, :]  # 右上：奇行偶列
        x2 = x[:, 0::2, 1::2, :]  # 左下：偶行奇列
        x3 = x[:, 1::2, 1::2, :]  # 右下：奇行奇列
        ...
        x = torch.cat([x0, x1, x2, x3], -1)  # B H/2 W/2 4*C  通道拼接与归一化，通道数变为4C
        x = x.view(B, H//2, W//2, 4 * C)  # B H/2*W/2 4*C
        
        # 线性降维，通道数压缩   4*dim  ——>  2*dim
        x = self.norm(x)
        x = self.reduction(x)
...
#         input = input + self.drop_path(self.ln_1(self.dcnv4(input)))  # DCNv4,自适应卷积模块
