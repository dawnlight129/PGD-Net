...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        if self.decoder:
            ...
            self.prompt1 = PromptGenBlock(prompt_dim=64,prompt_len=5,prompt_size = 64,lin_dim = 64)
            self.prompt2 = PromptGenBlock(prompt_dim=128,prompt_len=5,prompt_size = 32,lin_dim = 128)
            self.prompt3 = PromptGenBlock(prompt_dim=256,prompt_len=5,prompt_size = 16,lin_dim = 256)
    ...
    def forward(self, x, res1, res2, res3, res4, h, w):
        ...
        # print('inp解码3.shape',inp_dec_level3.shape)  # torch.Size([1, 320, 16, 16])
...
class AFENet(nn.Module):
    def __init__(self,
        ...
        self.Translayer_3 = BasicConv2d(4*mid_channel, 4*decode_channels, 1)
    ...
    def forward(self, x):
        ...
        #     x = x.repeat(1,3,1,1)
        # res1, res2, res3, res4 = self.vmencoder(x)

        # # 加转换
        # res1 = res1.permute(0, 3, 1, 2) # res1 [2, 96, 128, 128]
        # res2 = res2.permute(0, 3, 1, 2) # res2: torch.Size([2, 192, 64, 64])
        # res3 = res3.permute(0, 3, 1, 2) # res3: torch.Size([2, 384, 32, 32])
        # res4 = res4.permute(0, 3, 1, 2) # res4: torch.Size([2, 768, 16, 16])

        # res1 = self.Translayer_1(res1)  # res1: torch.Size([2, 64, 128, 128])
        # res2 = self.Translayer_2(res2)  # res2: torch.Size([2, 128, 64, 64])
        # res3 = self.Translayer_3(res3)  # res3: torch.Size([2, 256, 32, 32])
        # res4 = self.Translayer_4(res4)  # res4: torch.Size([2, 512, 16, 16])    
        # print('打印形状：res1:', res1.shape, 'res2:', res2.shape, 'res3:', res3.shape, 'res4:', res4.shape)

        # res2net骨干
        h, w = x.size()[-2:]
        x1 = self.cencoder.conv1(x)
        x1 = self.cencoder.bn1(x1)
        layer1 = self.cencoder.relu(x1)
        x1 = self.cencoder.maxpool(layer1)  
        layer2 = self.cencoder.layer1(x1)  
        layer3 = self.cencoder.layer2(layer2)  
        layer4 = self.cencoder.layer3(layer3)  
        layer5 = self.cencoder.layer4(layer4)
        
        res1 = self.Translayer_5(layer2)
        res2 = self.Translayer_6(layer3)
        res3 = self.Translayer_7(layer4)
        res4 = self.Translayer_8(layer5)
        # print('打印形状：res1:', res1.shape, 'res2:', res2.shape, 'res3:', res3.shape, 'res4:', res4.shape)
