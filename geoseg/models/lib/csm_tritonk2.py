...
def triton_cross_scan_flex_k2(
    ...
    i_h, i_w = tl.constexpr(i_hw // NW), tl.constexpr(i_hw % NW)   # 将i_h和i_w转换成Triton张量
...
def triton_cross_scan_flex_k2(
    ...
    i_h, i_w = tl.constexpr(i_hw // NW), tl.constexpr(i_hw % NW)
    
    # 创建形状为[1, 1]的张量，支持二维广播
    i_h_tensor = tl.full((1, 1), i_h, dtype=tl.int32)
    i_w_tensor = tl.full((1, 1), i_w, dtype=tl.int32)
    BH_tensor = tl.full((1, 1), BH, dtype=tl.int32)
    BW_tensor = tl.full((1, 1), BW, dtype=tl.int32)
    DH_tensor = tl.full((1, 1), DH, dtype=tl.int32)
    DW_tensor = tl.full((1, 1), DW, dtype=tl.int32)
    NH_tensor = tl.full((1, 1), NH, dtype=tl.int32)
    
    # 生成一维索引张量
    arange_h = tl.arange(0, BH)        # 形状[BH]
    arange_w = tl.arange(0, BW)        # 形状[BW]
    
    assert isinstance(arange_h, tl.tensor), "arange_h 必须是Triton张量"
    # 广播到[BH, BW]形状
    i_h_expanded = tl.broadcast_to(i_h_tensor, (BH, BW))
    i_w_expanded = tl.broadcast_to(i_w_tensor, (BH, BW))
    BH_expanded = tl.broadcast_to(BH_tensor, (BH, BW))
    BW_expanded = tl.broadcast_to(BW_tensor, (BH, BW))
    DH_expanded = tl.broadcast_to(DH_tensor, (BH, BW))
    DW_expanded = tl.broadcast_to(DW_tensor, (BH, BW))
    
    # 计算掩码（现在形状匹配）
    _mask_h = (i_h_expanded * BH_expanded + arange_h[:, None]) < DH_expanded
    _mask_w = (i_w_expanded * BW_expanded + arange_w[None, :]) < DH_expanded
    _mask_hw = _mask_h & _mask_w
    _for_C = min(DC - i_c * BC, BC)

    # 计算HWRoute0（所有项形状均为[BH, BW]）
    term1 = i_h_expanded * BH_expanded * DW_expanded
    term2 = arange_h[:, None] * DW_expanded
    term3 = i_w_expanded * BW_expanded
    term4 = arange_w[None, :]
    HWRoute0 = term1 + term2 + term3 + term4
    
    # 计算HWRoute2
    term1_r2 = (NH_tensor - i_h_expanded - 1) * BH_expanded * DW_expanded
    term2_r2 = (BH_expanded - 1 - arange_h[:, None]) * DW_expanded
    term3_r2 = (NW - i_w_expanded - 1) * BW_expanded
    term4_r2 = (BW_expanded - 1 - arange_w[None, :])
    term5_r2 = (DH_expanded - NH_tensor * BH_expanded) * DW_expanded
    term6_r2 = (DW_expanded - NW * BW_expanded)
    HWRoute2 = term1_r2 + term2_r2 + term3_r2 + term4_r2 + term5_r2 + term6_r2
