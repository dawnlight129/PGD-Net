...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.q_proj = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)  # 独立Q投影
        self.k_proj = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)  # 独立K投影
        self.v_proj = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)  # 独立V投影
    ...
    def forward(self, x):
        ...
        q = self.qkv_dwconv(self.q_proj(x))
        k = self.qkv_dwconv(self.k_proj(x))
        v = self.qkv_dwconv(self.v_proj(x))
