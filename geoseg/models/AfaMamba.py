...
from .pvtv2 import *
from .DSSA_MambaBlock import DeformMambaEncoderLayer
...
class ASFF(nn.Module):
    ...
    def forward(self, x_level_0, x_level_1, x_level_2, x_level_3):
        ...
        out_size = (out.shape[2], out.shape[3])  # 添加的代码

        if self.vis:
            return out,out_size, levels_weight, fused_out_reduced.sum(dim=1)
        else:
            return out,out_size
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.GS_1 = DeformMambaEncoderLayer(embed_dims=64,feedforward_channels=256)
        self.GS_2 = DeformMambaEncoderLayer(embed_dims=64,feedforward_channels=256)
        self.GS_3 = DeformMambaEncoderLayer(embed_dims=64,feedforward_channels=256)
        self.GS_4 = DeformMambaEncoderLayer(embed_dims=64,feedforward_channels=256)
    ...
    def forward(self, x, res1, res2, res3, res4, h, w):
        ...
        df_0, hw_shape0 = self.ASFF_0(x_sal4,x_sal3,x_sal2,x_sal1)
        df_1, hw_shape1 = self.ASFF_1(x_sal4,x_sal3,x_sal2,x_sal1)
        df_2, hw_shape2 = self.ASFF_2(x_sal4,x_sal3,x_sal2,x_sal1)
        df_3, hw_shape3 = self.ASFF_3(x_sal4,x_sal3,x_sal2,x_sal1)
        
        print("df_0.shape:", df_0.shape)
        #1/32尺度的输出
        df_0 = df_0.permute(0, 2, 3, 1)    # 添加
        x_out4 = self.GS_1(df_0, hw_shape0)
        ...
        #1/16尺度的输出
        sal3 = sal3.permute(0, 2, 3, 1)    # 添加
        x_out3 = self.GS_2(sal3, hw_shape1)
        ...
        #1/8尺度的输出
        sal2 = sal2.permute(0, 2, 3, 1)    # 添加
        x_out2 = self.GS_3(sal2, hw_shape2)
        
        #上采样2倍
        sal2_1 = F.interpolate(x_out2, size=x_sal1.size()[2:], mode='bilinear')
        sal1 = self.wf4(sal2_1, df_3)
        #1/4的输出
        sal1 = sal1.permute(0, 2, 3, 1)    # 添加
        x_out1 = self.GS_4(sal1, hw_shape3)   # 代码修改，添加了hw_shape参数
