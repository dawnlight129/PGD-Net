...
class OSSM(nn.Module):
    def __init__(
        ...
        print('是什么呢？d_inner:', d_inner,"d_model:", d_model)
    ...
    def forward(self, x, hw_shape):
        print("进入OSSM模块：")
        print(f"[OSSM] 输入形状: {x.shape}, hw_shape: {hw_shape}")
        ...
        # 打印中间层输入输出形状
        print(f"[OSSM] in_proj 前: {x.shape}")
        x = self.in_proj(x)  # 报错行
        print(f"[OSSM] in_proj 后: {x.shape}")
