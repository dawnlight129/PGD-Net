...
class SoftCrossEntropyLoss(nn.Module):
    ...
    - Handle tuple input (extract the last element as prediction)
    """
    ...
    def forward(self, input, target):
        ...
        if isinstance(input, tuple):
            # 假设最后一个元素是预测结果（如[features, logits]）
            input = input[-1]
        
        # 确保input是张量
        if not isinstance(input, Tensor):
            raise TypeError(f"Expected Tensor or tuple of Tensors, got {type(input)}")
